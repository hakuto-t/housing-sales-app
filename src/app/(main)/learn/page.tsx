...
function LearnContent() {
  ...
  const startSession = async (category?: string) => {
    ...
    try {
      const body: { count: number; category?: string } = { count: 10 };
      ...
      if (!res.ok) throw new Error("セッション開始に失敗しました");
      const data = await res.json();
  ...
  }, [phase, currentIndex, currentQuestion, resetAnswerState]);

  // Auto-start session from URL query param (e.g., /learn?category=建築基礎)
  useEffect(() => {
  ...
  const submitAnswer = async () => {
    ...
    // Retry up to 2 times with 20-second timeout each
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        if (attempt < 1 && isTimeout) {
          // First timeout: retry silently
          continue;
        }
        if (isTimeout) {
          setError("採点に時間がかかっています。もう一度お試しください。");
        } else {
          setError(err instanceof Error ? err.message : "エラーが発生しました");
        }
      }
  ...
  // Idle screen - category selection
  if (phase === "idle") {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">学習モード</h1>
        <p className="text-gray-500">カテゴリを選んでステップバイステップで学習、または全カテゴリで模擬試験ができます。</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {QUESTION_CATEGORIES.map((cat) => (
            <button
              ...
              className="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-200 bg-white hover:border-orange-400 hover:bg-orange-50 transition-all text-center group"
            >
              <span className="text-2xl">{categoryIcons[cat] || "📚"}</span>
              <span className="text-sm font-medium text-gray-700 group-hover:text-orange-700">{cat}</span>
            </button>
          ))}
        </div>

        {/* All categories - mock test */}
        <Card className="border-2 border-dashed border-gray-300 hover:border-orange-400 transition-colors cursor-pointer" onClick={() => startSession()}>
          <CardContent className="p-6 text-center">
            <div className="flex items-center justify-center gap-3">
              <span className="text-2xl">🎯</span>
              <div className="text-left">
                <p className="font-bold text-gray-900">全カテゴリ（模擬試験）</p>
                <p className="text-sm text-gray-500">苦手分野を中心にランダム出題</p>
              </div>
            </div>
          </CardContent>
        </Card>
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-gray-500">問題を準備中...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">
          学習完了{selectedCategory && ` - ${selectedCategory}`}
        </h1>
          ...
          <CardContent className="p-8 text-center space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              <Button
                ...
                onClick={() => { setPhase("idle"); setError(""); setSelectedCategory(null); }}
              >
                もう一度学習する
              </Button>
              <Button
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">
          学習モード{selectedCategory && ` - ${selectedCategory}`}
        </h1>
        <div className="flex items-center gap-4">
          <Badge variant="secondary" className="text-sm">
            {currentIndex + 1} / {questions.length}
          </Badge>
          {result && result.comboCount > 0 && (
            <Badge className="bg-orange-500 text-white text-sm">
              {result.comboCount}コンボ
            </Badge>
          )}
        </div>
      </div>

      <Progress value={currentIndex + 1} max={questions.length} className="h-2" />
          ...
          <CardContent className="space-y-4">
            ...
            {phase === "feedback" && result && (
              <div className="mt-6 space-y-4">
                ...
                {/* Missing points - what the user's answer lacked */}
                ...
                {result.modelAnswer && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <p className="text-sm text-green-700 leading-relaxed">{result.modelAnswer}</p>
                  </div>
                )}

                {/* Must-include keywords reference */}
                ...
                {/* Explanation - deep learning section */}
                {result.explanation && (
